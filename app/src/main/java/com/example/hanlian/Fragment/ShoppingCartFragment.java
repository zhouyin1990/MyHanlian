...
//				Log.e("infolist==",infolist.toString());
                ...
                for (int i = 0; i < infolist.size(); i++) {
                    ...
                    if (ischeck) {
                        ...
                        try {
                            object3.put("title", infolist.get(i).getTitle());
                            object3.put("ImgURL", infolist.get(i).getImgURL());
                            object3.put("totalprice", infolist.get(i).getTotalprice());
                            object3.put("goodsid", infolist.get(i).getGoodsid());
                            object3.put("count", infolist.get(i).getCount());
                            object3.put("shoppingName", infolist.get(i).getShoppingName());
                            object3.put("shoppingID", infolist.get(i).getShoppingID());
                            object3.put("Zongjia", infolist.get(i).getZongjia());

                            String GOODSLIST = infolist.get(i).getGOODSLIST();
                            JSONObject goodsObject = new JSONObject(GOODSLIST);

                            JSONArray goodsJson = goodsObject.getJSONArray("GOODSLIST");

                            object3.put("CM_SELLERID", goodsJson.getJSONObject(0).getString("CM_SELLERID"));
                            object3.put("GOODSID", goodsJson.getJSONObject(0).getString("GOODSID"));

                            JSONArray GOODSDETAILS = goodsJson.getJSONObject(0).getJSONArray("GOODSDETAILS");
                            ...
                            for (int j = 0; j < GOODSDETAILS.length(); j++) {
                                ...
                                array2.put("GOODSDETAILSID", GOODSDETAILS.getJSONObject(j).getString("GOODSDETAILSID"));
                                array2.put("SPEC_NUMBER", GOODSDETAILS.getJSONObject(j).getString("SPEC_NUMBER"));
                                array2.put("COLOR", GOODSDETAILS.getJSONObject(j).getString("COLOR"));
                                array4.put(j, array2);
                            }

                            object3.put("GOODSLIST", array4);
...
//						Log.e("object4==",object4.toString());

                        if (allcouts > 0 && allprice > 0) {
                    ...
                    case 0:
                        if (infolist.size() != 0) {
                            ...
                            if (select_all.isChecked()) {
                                cardAdapter.numchangelistener.onNumberChange(-infolist.get(position).getCount());
                                cardAdapter.ontotalpricechangelistener.TotalPriceChange(-infolist.get(position).getTotalprice());
                            }
...
//                hold.imageChange = (ImageView) view.findViewById(R.id.image_change);
            ...
            hold.checkBox.setOnCheckedChangeListener(new OnCheckedChangeListener() {
                ...
                @Override
                public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                    //CompoundButton 符合按钮      这里是选中状态 isChecked ==true
...
//            });
    ...
    private ArrayList<DetailsModle.Result> checkResultList = new ArrayList<DetailsModle.Result>();
